...
class WCAI_Settings {
	...
	public static function handle_reindex(): void {
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_die( esc_html__( 'Unauthorized', 'shopask-ai-shopping-assistant' ) );
		}
	...
	public static function maybe_reindex_notice(): void {
		...
		if ( 'queued' === $status ) {
			...
			echo esc_html__( 'Catalog reindex has been queued. Progress runs in the background via Action Scheduler.', 'shopask-ai-shopping-assistant' );
			echo '</p></div>';
		} elseif ( 'no_key' === $status ) {
			echo '<div class="notice notice-error is-dismissible"><p>';
			echo esc_html__( 'Add an API key before reindexing (or use LongCat/Claude with local embeddings).', 'shopask-ai-shopping-assistant' );
			echo '</p></div>';
		} elseif ( 'as_missing' === $status ) {
			echo '<div class="notice notice-error is-dismissible"><p>';
			echo esc_html__( 'Action Scheduler is not available. Ensure WooCommerce is active and up to date, then try again.', 'shopask-ai-shopping-assistant' );
			echo '</p></div>';
		}
	}
	...
	public static function render_page(): void {
		...
		echo '<h1>' . esc_html__( 'ShopAsk AI – Shopping Assistant for WooCommerce', 'shopask-ai-shopping-assistant' ) . '</h1>';
	...
	private static function render_settings_tab(): void {
		if ( ! class_exists( 'WCAI_Providers' ) ) {
			...
			echo esc_html__( 'Plugin files are incomplete (providers missing). Reinstall the plugin.', 'shopask-ai-shopping-assistant' );
			echo '</p></div>';
			return;
		}
		...
		<form method="post" action="options.php">
			...
			<section class="wcai-card">
				<h2><?php esc_html_e( 'API & models', 'shopask-ai-shopping-assistant' ); ?></h2>
				<p class="description"><?php esc_html_e( 'Connect a provider, then test before indexing.', 'shopask-ai-shopping-assistant' ); ?></p>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="wcai_provider"><?php esc_html_e( 'AI provider', 'shopask-ai-shopping-assistant' ); ?></label></th>
						<td>
							...
							<p class="description" id="wcai_provider_hint"><?php esc_html_e( 'Claude and LongCat use local embeddings (no embedding API). OpenAI, Gemini, and OpenRouter can use API embeddings.', 'shopask-ai-shopping-assistant' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="wcai_api_key"><?php esc_html_e( 'API Key', 'shopask-ai-shopping-assistant' ); ?></label></th>
						<td>
							<input type="password" class="regular-text" id="wcai_api_key" name="wcai_settings[api_key]" value="" placeholder="<?php echo $settings['api_key'] ? esc_attr__( '•••••••• (saved — leave blank to keep)', 'shopask-ai-shopping-assistant' ) : ''; ?>" autocomplete="off" />
							<p class="description" id="wcai_key_hint"></p>
							<p>
								<button type="button" class="button button-secondary" id="wcai-test-connection"><?php esc_html_e( 'Test connection', 'shopask-ai-shopping-assistant' ); ?></button>
								<span id="wcai-test-result" class="wcai-callout" aria-live="polite"></span>
							</p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="wcai_api_base"><?php esc_html_e( 'API base URL', 'shopask-ai-shopping-assistant' ); ?></label></th>
						<td>
							<input type="url" class="regular-text" id="wcai_api_base" name="wcai_settings[api_base]" value="<?php echo esc_attr( $settings['api_base'] ?? '' ); ?>" />
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="wcai_chat_model"><?php esc_html_e( 'Chat model', 'shopask-ai-shopping-assistant' ); ?></label></th>
						<td>
							...
							<p class="description"><?php esc_html_e( 'Pick from the list or type a custom model ID in the text field.', 'shopask-ai-shopping-assistant' ); ?></p>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="wcai_embedding_mode"><?php esc_html_e( 'Embeddings', 'shopask-ai-shopping-assistant' ); ?></label></th>
						<td>
							<select id="wcai_embedding_mode" name="wcai_settings[embedding_mode]">
								<option value="auto" <?php selected( $settings['embedding_mode'] ?? 'auto', 'auto' ); ?>><?php esc_html_e( 'Auto (recommended)', 'shopask-ai-shopping-assistant' ); ?></option>
								<option value="local" <?php selected( $settings['embedding_mode'] ?? '', 'local' ); ?>><?php esc_html_e( 'Local only', 'shopask-ai-shopping-assistant' ); ?></option>
								<option value="api" <?php selected( $settings['embedding_mode'] ?? '', 'api' ); ?>><?php esc_html_e( 'API embeddings', 'shopask-ai-shopping-assistant' ); ?></option>
							</select>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="wcai_embedding_model"><?php esc_html_e( 'Embedding model', 'shopask-ai-shopping-assistant' ); ?></label></th>
						<td>
			...
			<section class="wcai-card">
				<h2><?php esc_html_e( 'Widget & placement', 'shopask-ai-shopping-assistant' ); ?></h2>
				<p class="description"><?php esc_html_e( 'Floating bubble, search bar, shortcodes, and accent.', 'shopask-ai-shopping-assistant' ); ?></p>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><?php esc_html_e( 'Enable assets', 'shopask-ai-shopping-assistant' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="wcai_settings[widget_enabled]" value="1" <?php checked( $settings['widget_enabled'], '1' ); ?> />
								<?php esc_html_e( 'Load assistant scripts on the storefront', 'shopask-ai-shopping-assistant' ); ?>
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><?php esc_html_e( 'Floating button', 'shopask-ai-shopping-assistant' ); ?></th>
						<td>
							<label>
					...
					<tr>
						<th scope="row"><label for="wcai_auto_search"><?php esc_html_e( 'Auto-insert search bar', 'shopask-ai-shopping-assistant' ); ?></label></th>
						<td>
							<select id="wcai_auto_search" name="wcai_settings[auto_search_location]">
								<option value="none" <?php selected( $settings['auto_search_location'] ?? 'none', 'none' ); ?>><?php esc_html_e( 'Off — place manually', 'shopask-ai-shopping-assistant' ); ?></option>
								<option value="body_open" <?php selected( $settings['auto_search_location'] ?? '', 'body_open' ); ?>><?php esc_html_e( 'Top of page (after body opens / near header)', 'shopask-ai-shopping-assistant' ); ?></option>
							</select>
							<p class="description"><?php esc_html_e( 'Or place widgets yourself with shortcodes / the Gutenberg block (hero, nav area, product pages, etc.).', 'shopask-ai-shopping-assistant' ); ?></p>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<th scope="row"><?php esc_html_e( 'White-label', 'shopask-ai-shopping-assistant' ); ?></th>
						<td>
							<label>
								<input type="checkbox" name="wcai_settings[hide_branding]" value="1" <?php checked( $settings['hide_branding'], '1' ); ?> />
								<?php esc_html_e( 'Hide “Powered by” branding', 'shopask-ai-shopping-assistant' ); ?>
							</label>
						</td>
					</tr>
				</table>
			...
			<section class="wcai-card">
				<h2><?php esc_html_e( 'Retrieval & indexing', 'shopask-ai-shopping-assistant' ); ?></h2>
				<p class="description"><?php esc_html_e( 'Tune how products are ranked and how many candidates the assistant considers.', 'shopask-ai-shopping-assistant' ); ?></p>
				<table class="form-table" role="presentation">
					<tr>
						<th scope="row"><label for="wcai_index_mode"><?php esc_html_e( 'Index mode', 'shopask-ai-shopping-assistant' ); ?></label></th>
						<td>
							<select id="wcai_index_mode" name="wcai_settings[index_mode]">
								<option value="parent" <?php selected( $settings['index_mode'], 'parent' ); ?>><?php esc_html_e( 'Parent products', 'shopask-ai-shopping-assistant' ); ?></option>
								<option value="variation" <?php selected( $settings['index_mode'], 'variation' ); ?>><?php esc_html_e( 'Variations (size/color rows)', 'shopask-ai-shopping-assistant' ); ?></option>
							</select>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="wcai_top_n"><?php esc_html_e( 'Candidates (top N)', 'shopask-ai-shopping-assistant' ); ?></label></th>
					...
					<tr>
						<th scope="row"><label for="wcai_threshold"><?php esc_html_e( 'Similarity threshold', 'shopask-ai-shopping-assistant' ); ?></label></th>
					...
					<tr>
						<th scope="row"><label for="wcai_prefilter"><?php esc_html_e( 'Prefilter limit', 'shopask-ai-shopping-assistant' ); ?></label></th>
						...
						<p class="description"><?php esc_html_e( 'Max rows to cosine-rank after SQL/FULLTEXT narrowing (in-DB scale).', 'shopask-ai-shopping-assistant' ); ?></p></td>
					</tr>
					<tr>
						<th scope="row"><label for="wcai_daily_cap"><?php esc_html_e( 'Daily query limit', 'shopask-ai-shopping-assistant' ); ?></label></th>
						<td>
							...
							<p class="description"><?php esc_html_e( 'Soft cap on storefront AI queries per UTC day (default 500). Uses your provider API key. Set 0 for unlimited (not recommended).', 'shopask-ai-shopping-assistant' ); ?></p>
						</td>
					</tr>
				</table>
				<?php submit_button( __( 'Save settings', 'shopask-ai-shopping-assistant' ) ); ?>
			</section>
		</form>

		<section class="wcai-card">
			<h2><?php esc_html_e( 'Catalog index', 'shopask-ai-shopping-assistant' ); ?></h2>
			<p class="description">
				<?php
				printf(
					/* translators: %d: number of indexed products */
					esc_html__( 'Currently indexed rows: %d', 'shopask-ai-shopping-assistant' ),
					(int) $count
				);
				?>
			</p>
				...
				<p id="wcai-reindex-label" class="description">
					...
					echo esc_html(
						sprintf(
							...
							__( 'Progress: %1$d / %2$d (%3$s)', 'shopask-ai-shopping-assistant' ),
							$done,
							$total,
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				...
				<?php submit_button( __( 'Reindex Catalog', 'shopask-ai-shopping-assistant' ), 'secondary', 'submit', false ); ?>
			</form>
		</section>
		<?php
	}
